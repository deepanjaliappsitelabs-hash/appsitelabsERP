...
const updateWorkLog = async (id, data = {}) => {
  ...
  const [result] = await pool.query(
    `UPDATE work_logs
     SET work_date = ?, work_mode = ?, tasks = ?, blockers = ?, tomorrow_plan = ?,
         mood = ?, productivity = ?, links = ?, total_hours = ?
     WHERE id = ?`,
    [
      data.date,
      data.workMode || "",
...
module.exports = { createWorkLog, getByEmployee, getById, getAll, updateWorkLog, deleteWorkLog };
