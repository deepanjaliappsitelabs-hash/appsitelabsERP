...
import { createWorkLog, deleteWorkLog, getMyWorkLogs, updateWorkLog } from "../../services/workLogService";
...
export default function DailyWorkLog() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setHistory((prev) => editingLogId
        ? prev.map((item) => (String(item.id || item._id) === String(editingLogId) ? log : item))
        : [log, ...prev]);
      setSubmitted(true);
    } catch (err) {
      const message = err.response?.data?.message || err.message;
      toast.error(message ? `Work log save failed: ${message}` : "Work log save failed. Please try again.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleEdit = (log) => {
    setTasks((log.tasks || []).map((task, index) => ({ ...task, id: task.id || `${Date.now()}-${index}` })));
    setBlockers(log.blockers || "");
    setTomorrowPlan(log.tomorrowPlan || "");
    setWorkMode(log.workMode || WORK_MODE[0]);
    setMood(log.mood || null);
    setProductivity(Number(log.productivity) || 0);
    setLinks(log.links || "");
    setErrors({});
    setEditingLogId(log.id || log._id);
    setLogDate(log.date || new Date().toISOString().slice(0, 10));
    setActiveTab("form");
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteWorkLog(deleteTarget.id || deleteTarget._id);
      setHistory((prev) => prev.filter((log) => String(log.id || log._id) !== String(deleteTarget.id || deleteTarget._id)));
    ...
    <div className="min-h-screen bg-[#F6F7FB] px-4 py-8 sm:px-8">
      <div className="mx-auto max-w-3xl space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold text-slate-900">{editingLogId ? "Edit Work Log" : "Daily Work Log"}</h1>
        ...
        {activeTab === "form" && (
          <div className="space-y-5">
            ...
            <button
              ...
              disabled={saving}
              className="flex w-full items-center justify-center gap-2 rounded-2xl bg-[#302568] py-4 text-sm font-bold text-white shadow-md shadow-[#302568]/20 transition hover:bg-[#3d3080] active:bg-[#251d52] disabled:cursor-not-allowed disabled:opacity-60"
            >
              <FiSend /> {saving ? "Saving..." : editingLogId ? "Save Changes" : "Submit Today's Work Log"}
            </button>
          </div>
        )}
              ...
              history.map((log) => (
                <div key={log.id} className="rounded-2xl border border-[#E7E8F0] bg-white shadow-sm">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                      ...
                      <button type="button" onClick={() => handleEdit(log)} className="inline-flex items-center gap-1 rounded-full bg-blue-50 px-3 py-1 text-xs font-semibold text-blue-700 hover:bg-blue-100">
                        <FiEdit2 /> Edit
                      </button>
                      <button type="button" onClick={() => setViewLog(log)} className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-200">
                        <FiEye /> View
                      </button>
                      <button type="button" onClick={() => setDeleteTarget(log)} className="inline-flex items-center gap-1 rounded-full bg-red-50 px-3 py-1 text-xs font-semibold text-red-700 hover:bg-red-100">
                        <FiTrash2 /> Delete
                      </button>
                    </div>
                  </div>
      ...
      {viewLog && (
        <Modal
          title={`Work Log — ${viewLog.date}`}
          onClose={() => setViewLog(null)}
          className="max-w-3xl"
        >
          <div className="space-y-4">
            <div className="flex flex-wrap gap-2 text-xs font-semibold">
              <span className="rounded-full bg-[#F5F3FC] px-3 py-1 text-[#302568]">{viewLog.workMode || "Work mode not set"}</span>
              <span className="rounded-full bg-amber-50 px-3 py-1 text-amber-700">⭐ {viewLog.productivity || 0}/5</span>
              <span className="rounded-full bg-slate-100 px-3 py-1 text-slate-600">{viewLog.mood || "Mood not set"}</span>
            </div>
            {(viewLog.tasks || []).map((task, index) => (
              <div key={index} className="rounded-xl border border-[#E7E8F0] p-4">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <p className="font-semibold text-slate-800">{task.taskName || "Untitled task"}</p>
                  <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${STATUS_BADGE[task.status] || "bg-slate-50 text-slate-500"}`}>{task.status || "Unknown"} · {task.hours || 0}h</span>
                </div>
                <p className="mt-1 text-sm text-slate-500">{task.projectName || "No project"}</p>
                {task.description && <p className="mt-3 text-sm text-slate-600">{task.description}</p>}
              </div>
            ))}
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="rounded-xl bg-red-50 px-4 py-3"><p className="text-xs font-bold uppercase text-red-500">Blockers</p><p className="mt-1 text-sm text-red-700">{viewLog.blockers || "None"}</p></div>
              <div className="rounded-xl bg-blue-50 px-4 py-3"><p className="text-xs font-bold uppercase text-blue-500">Tomorrow's Plan</p><p className="mt-1 text-sm text-blue-700">{viewLog.tomorrowPlan || "-"}</p></div>
            </div>
            {viewLog.links && <p className="break-all text-sm text-[#302568]">{viewLog.links}</p>}
          </div>
        </Modal>
      )}
    </div>
